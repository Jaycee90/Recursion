...
void Merge2AscListsRecur(Node*& headPtrX, Node*& headPtrY, Node*& headPtrZ)
{
   //Check if both X-list and Y-list are empty
   //If so, the function returns immediately.
   if (headPtrX == 0 && headPtrY == 0) return;
   
   //checks if either headPtrX or headPtrY is nullptr.
   else if (headPtrX == 0)
   {
   	  //If headPtrX is nullptr, the entire Y-list is appended 
      //to the end of Z-list, and the function returns
      headPtrZ = headPtrY;
      headPtrY = 0;
      return;
    } 
	...
	{
	   //If headPtrY is nullptr, the entire X-list is appended
	   //to the end of Z-list, and the function returns
       headPtrZ = headPtrX;
       headPtrX = 0;
       return;
	...
	{
	   //If neither list is empty, compares the values at 
	   //the heads of the X-list and Y-list, and appends 
	   //the smaller value to the end of the Z-list.
       if (headPtrX->data < headPtrY->data)
	   {
        ...
        //Call the function recursively with the updated head
		//pointers until the X-list or Y-list becomes empty.
        Merge2AscListsRecur(headPtrX, headPtrY, headPtrZ->link);
    }
}
